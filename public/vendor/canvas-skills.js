(function() {
    ...
    function A(aH, aK) {
        var j = [],
            aI = aH.length,
            aJ;
        for (aJ = 0; aJ < aI; ++aJ) { j.push(aH[aJ]) }
        j.sort(aK);
        return j
    }
    ...
    function U(aK, i) {
        ...
        if (aK[0] === "#") {
            if (!h[aK]) { if (aK.length === 4) { h[aK] = "rgba(" + m[aK[1]] + m[aK[2]] + m[aK[3]] } else { h[aK] = "rgba(" + l[aK.substr(1, 2)] + l[aK.substr(3, 2)] + l[aK.substr(5, 2)] } }
            aJ = h[aK] + j
        } else {
    ...
    function ak(aL, j, aK, aJ) {
        var aI = aL.createLinearGradient(0, 0, j, 0),
            aH;
        for (aH in aJ) { aI.addColorStop(1 - aH, aJ[aH]) }
        aL.fillStyle = aI;
    ...
    function k(aJ, aH, j) {
        ...
        if (aJ.gCanvas) {
            ...
            if (I(aM[0])) { aN = aM.length } else { aM = [aM] }
            aJ.gCanvas = aL = P(aI, aN);
            if (!aL) { return null }
            aP = aL.getContext("2d");
            for (aK = 0; aK < aN; ++aK) { ak(aP, aI, aK, aM[aK]) }
        }
        j = s(aD(j || 0, aN - 1), 0);
    ...
    function X(aQ, aJ, j, aU, aT, aR, aP, aL, aI, aS, aK, aO) {
        ...
        for (aM = 0; aM < aU.length; ++aM) {
            aV = 0;
            if (aK) { if ("right" == aO) { aV = aS - aK[aM] } else { if ("centre" == aO) { aV = (aS - aK[aM]) / 2 } } }
        ...
        if (aK.txtOpt) {
            ...
            if (!aP.HasText()) {
                ...
                if (aK.bgColour || aK.bgOutlineThickness) {
                    ...
                    aJ = aP.fimage.width;
                    aO = aP.fimage.height;
                    if (aK.outlineMethod == "colour") { aI = aC(aP.fimage, aJ, aO, aN, aM, aK.bgOutlineThickness, aP.outline.colour, aK.padding, aK.bgRadius, 1); if (aI) { aP.oimage = aI.image } }
    ...
    function u(j, aI, aH) {
        var i = 1,
            aJ;
        if (aI) { i = 1 * (j.getAttribute(aI) || aH) } else { if (aJ = Y(j, "font-size")) { i = (aJ.indexOf("px") > -1 && aJ.replace("px", "") * 1) || (aJ.indexOf("pt") > -1 && aJ.replace("pt", "") * 1.25) || aJ * 3.3 } }
        return i
    }
    ...
    x.SetMethod = function(aH) {
        var j = { block: ["PreDraw", "DrawBlock"], colour: ["PreDraw", "DrawColour"], outline: ["PostDraw", "DrawOutline"], classic: ["LastDraw", "DrawOutline"], size: ["PreDraw", "DrawSize"], none: ["LastDraw"] },
            i = j[aH] || j.outline;
        if (aH == "none") { this.Draw = function() { return 1 } } else { this.drawFunc = this[i[1]] }
        this[i[0]] = this.Draw
    };
    ...
    x.DrawSize = function(aO, aR, aP, aS, aM, j, aT, aI, aQ) {
        var aL = aT.w,
            ...
            aJ, aK, aN;
        if (this.pulsate) {
            if (aT.image) { aN = (aT.image.height + this.tc.outlineIncrease) / aT.image.height } else { aN = aT.oscale }
    ...
    x.DrawColourImage = function(aM, aP, aN, aQ, aL, i, aT, j, aO) {
        ...
        if (this.pulsate) { aT.alpha = 1 - at(this.pulse, 2) } else { aT.alpha = 1 }
        aT.Draw(aM, j, aO);
    ...
    c.Measure = function(aM, aP) {
        ...
        if (aP.txtOpt) {
            ...
            if (this.image) { aK.SetImage(this.image, this.iw, this.ih, aP.imagePosition, aP.imagePadding, aP.imageAlign, aP.imageVAlign, aP.imageScale) }
            ...
            if (aP.outlineMethod == "colour") { this.oimage = aK.Create(this.outline.colour, this.bgColour, this.outline.colour, aQ * this.bgOutlineThickness, aP.shadow, aQ * aP.shadowBlur, j, aQ * this.padding, aQ * this.bgRadius) } else {
                if (aP.outlineMethod == "size") {
                    ...
                    if (this.image) { aK.SetImage(this.image, this.iw + aP.outlineIncrease, this.ih + aP.outlineIncrease, aP.imagePosition, aP.imagePadding, aP.imageAlign, aP.imageVAlign, aP.imageScale) }
    ...
    c.DrawText = function(aJ, aM, aI) {
        ...
        for (j = 0; j < this.text.length; ++j) {
            aH = aL;
            if ("right" == aN.textAlign) { aH += this.w / 2 - this.line_widths[j] } else { if ("centre" == aN.textAlign) { aH -= this.line_widths[j] / 2 } else { aH -= this.w / 2 } }
    ...
    c.Clicked = function(aK) {
        var j = this.a,
            ...
            i;
        if (aH != "" && aH != "_self") {
            if (self.frames[aH]) { self.frames[aH].document.location = aI } else {
                try { if (top.frames[aH]) { top.frames[aH].document.location = aI; return } } catch (aJ) {}
                window.open(aI, aH)
            }
            return
        }
    ...
    function y(aN, j, aI) {
        ...
        if (aM && (!aM.getContext || !aM.getContext("2d").fillText)) {
            ...
            for (aH = 0; aH < aJ.length; ++aH) { aK[aJ[aH]] = aM[aJ[aH]] }
    ...
    Q.CreateTag = function(aL) {
        ...
        if ("image" != this.imageMode) {
            ...
            if (!aN.Empty()) {
                ...
                if (this.splitWidth) { aK = aN.SplitWidth(this.splitWidth, this.ctxt, aH, this.textHeight) }
    ...
    Q.Weight = function(aN) {
        ...
        for (aH = 0; aH < aJ; ++aH) {
            ...
            for (aO = 0; aO < aM; ++aO) {
                ...
                if (!this.max_weight[aO] || aL > this.max_weight[aO]) { this.max_weight[aO] = aL }
    ...
    Q.Load = function() {
        ...
        if (aR.length) {
            aK.length = aR.length;
            for (aO = 0; aO < aR.length; ++aO) { aK[aO] = aO }
            this.shuffleTags && an(aK);
            aI = 100 * this.radiusX;
            aH = 100 * this.radiusY;
            j = 100 * this.radiusZ;
            this.max_radius = s(aI, s(aH, j));
            for (aO = 0; aO < aR.length; ++aO) { aQ = this.CreateTag(aR[aK[aO]]); if (aQ) { aM.push(aQ) } }
            this.weight && this.Weight(aM, true);
            if (this.shapeArgs) { this.shapeArgs[0] = aM.length } else {
                aL = this.shape.toString().split(/[(),]/);
                aP = aL.shift();
                if (typeof window[aP] === "function") { this.shape = window[aP] } else { this.shape = aN[aP] || aN.sphere }
    ...
    Q.Update = function() {
        ...
        if (aQ.length) {
            ...
            aL = 0;
            while (aO.length) { aP.splice(ap(++aL * aJ), 0, this.CreateTag(aQ[aO.shift()])) }
            this.shapeArgs[0] = aH = aP.length;
            aI = this.shape.apply(this, this.shapeArgs);
            for (aL = 0; aL < aH; ++aL) { aP[aL].position = new ae(aI[aL][0], aI[aL][1], aI[aL][2]) }
    ...
    Q.Draw = function(aR) {
        ...
        var aL = this.canvas,
            ...
            aM;
        this.time = aR;
        if (this.frozen && this.drawn) { return this.Animate(aJ, aQ, aI) }
        aM = this.AnimateFixed();
        aX.setTransform(1, 0, 0, 1, 0, 0);
        for (aV = 0; aV < aU; ++aV) { aK[aV].Calc(this.transform, this.fixedAlpha) }
    ...
    Q.SetTTDiv = function(aI, j) {
        var i = this,
            aH = i.ttdiv.style;
        if (aI != i.ttdiv.innerHTML) { aH.display = "none" }
    ...
    Q.FindTag = function(aH) {
        ...
        if (!I(aH)) { return this.taglist[aH] }
        var aI, aJ, j;
        if (ai(aH.id)) { aI = "id", aJ = aH.id } else { if (ai(aH.text)) { aI = "innerText", aJ = aH.text } }
        for (j = 0; j < this.taglist.length; ++j) { if (this.taglist[j].a[aI] == aJ) { return this.taglist[j] } }
    };
    ...
    y.Delete = function(aI) {
        var j, aH;
        if (b[aI]) { aH = C.getElementById(aI); if (aH) { for (j = 0; j < b[aI].length; ++j) { a(b[aI][j][0], b[aI][j][1], aH) } } }
        delete b[aI];
        delete y.tc[aI]
    };
